...
export default function ChartsDashboard({ tasks }: Props) {
  const revenueByPriority = ["High", "Medium", "Low"].map((p) => ({
    ...
    revenue: tasks
      ...
      .reduce((sum, t) => {
        ...
        return Number.isFinite(rev) ? sum + rev : sum;
      }, 0),
  }));
  ...
  const roiBuckets = [
    {
      label: "<200",
      count: tasks.filter((t) => typeof t.roi === "number" && t.roi < 200)
        .length,
    },
    {
      label: "200-500",
    ...
    {
      label: ">500",
      count: tasks.filter((t) => typeof t.roi === "number" && t.roi > 500)
        .length,
    },
    {
      label: "N/A",
      count: tasks.filter((t) => t.roi == null).length,
    },
  ];
          ...
          <Box>
            ...
            <BarChart
              ...
              xAxis={[
                {
                  scaleType: "band",
                  data: revenueByPriority.map((d) => d.priority),
                },
              ]}
              series={[
                {
